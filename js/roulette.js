class SetMyMapClass{
  constructor(){
    ...
    this.geojson_poly = L.geoJSON(countryPoly, {//ポリゴンデータの読み込み
  ...
  highlightFeature (e) {//マウスホバーしたポリゴンに対して境界線の強調表示を行う
    layer = e.target;
  ...
  onEachFeature (feature, layer) {
    layer.on({
      mouseover: this.highlightFeature,//強調表示する
      mouseout: this.resetHighlight,//強調表示を消す
      click: this.zoomClickFeature,//クリックしたらズームして特定のポリゴンの強調表示
    });
    console.log('onEach')
  ...
  zoomClickFeature (e) {// クリックした国を全体表示してズーム
    center = [[e.target._bounds._northEast.lat,e.target._bounds._northEast.lng],[e.target._bounds._southWest.lat,e.target._bounds._southWest.lng]]
    this.geojson_poly.setStyle({
      fillOpacity:0.6,
      // stroke:false
    })
...
class SelectClass {
  ...
  zoomLastSelectCountry(layer){
    SetMyMap.geojson_poly.eachLayer(layer => {
      if(layer.feature.properties.jp_name == this.selectLastCountry){
        SetMyMap.mymap.setView([layer.feature.properties.lat,layer.feature.properties.lon],4);//セレクトされた国をズーム
      }
    })
  }
